...
_ENV_PATH = "VANTAGE_HYPOTHESES_PATH"
...
def default_hypotheses_path() -> Path:
    ...
    Returns:
        Env override path when ``VANTAGE_HYPOTHESES_PATH`` is set,
        otherwise ``~/.vantage/hypotheses.json``.
    """
    override = get_env_config().paths.vantage_hypotheses_path.strip()
    if override:
        return Path(override).expanduser()
    return Path.home() / ".vantage" / "hypotheses.json"
...
class Hypothesis:
    ...
    Attributes:
        ...
        skills: Relevant Vantage skills.
