...
def _get_codex_token() -> Any:
    try:
        ...
        raise RuntimeError(
            ...
            "vantage provider login openai-codex"
        ) from exc
    try:
        token = get_token()
    except Exception as exc:
        raise RuntimeError("OpenAI Codex is not logged in. Run: vantage provider login openai-codex") from exc
    if not (token and getattr(token, "access", None) and getattr(token, "account_id", None)):
        raise RuntimeError("OpenAI Codex is not logged in. Run: vantage provider login openai-codex")
    return token
...
class OpenAICodexLLM:
    """Minimal LangChain-compatible adapter for Vantage's ChatLLM."""
