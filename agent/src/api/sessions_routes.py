...
# Upper bound on a single chat message, in characters (~150k tokens).
#
# This is the largest value that still WORKS end-to-end, measured against the
# configured provider rather than guessed: 600k chars round-trips, while 800k
# (~199k tokens) and above are rejected with `context_too_large`. Raising the
# cap past this point does not buy capacity — it only moves the failure from a
# clear 422 here to an opaque provider error mid-turn, after the user has
# already waited.
#
# Note this is a *ceiling*, not a working budget: a message near it consumes
# essentially the whole context, leaving no room for history or tool results.
# Bulk data belongs in an uploaded file (POST /upload, 50 MB), which
# `read_document` streams back in bounded windows (see doc_reader_tool) so a
# file of any size can be processed a chunk at a time.
...
class SendMessageRequest(BaseModel):
    ...
    client_request_id: Optional[str] = Field(
        ...
        description="Stable client-generated key used to safely retry a send.",
    )
    ...
    async def send_message(session_id: str, payload: SendMessageRequest, http_request: Request):
        ...
        try:
            ...
            if payload.client_request_id:
                retry_kwargs["client_request_id"] = payload.client_request_id
