...
def _set_owner_only_secret_permissions(path: Path, *, fd: int | None = None) -> None:
    """Restrict a secret file to the trusted local users for this workspace.

    POSIX keeps conventional owner-only ``0600`` permissions. On Windows the
    ...
    """
    if os.name != "nt":
        if fd is not None and callable(getattr(os, "fchmod", None)):
            os.fchmod(fd, 0o600)
        else:
            os.chmod(path, 0o600)
        return

    # Windows' chmod only controls the read-only bit; use a protected NTFS DACL
    # so inherited Users/Everyone entries cannot read provider credentials.
    try:
        ...
        logger.warning("pywin32 unavailable; secret file keeps inherited Windows ACLs")
        return

    token = win32security.OpenProcessToken(win32api.GetCurrentProcess(), win32con.TOKEN_QUERY)
    user_sid = win32security.GetTokenInformation(token, win32security.TokenUser)[0]
    allowed_sids = [user_sid]
    account_name = "\\".join(
        part for part in (os.getenv("USERDOMAIN", ""), os.getenv("USERNAME", "")) if part
    )
    if account_name:
        try:
            interactive_sid = win32security.LookupAccountName(None, account_name)[0]
            known = {
                win32security.ConvertSidToStringSid(sid)
                for sid in allowed_sids
            }
            if win32security.ConvertSidToStringSid(interactive_sid) not in known:
                allowed_sids.append(interactive_sid)
        except Exception:  # noqa: BLE001 - current token remains the secure fallback
            pass
    ...
    for sid in allowed_sids:
        dacl.AddAccessAllowedAceEx(win32security.ACL_REVISION, 0, access_mask, sid)
...
def _atomic_write_secret(path: Path, content: str) -> None:
    ...
    try:
        ...
        try:
            _set_owner_only_secret_permissions(path, fd=fd)
            os.write(fd, data)
        finally:
            os.close(fd)
        return
    try:
        try:
            _set_owner_only_secret_permissions(Path(tmp), fd=fd)
            os.write(fd, data)
            os.fsync(fd)
        finally:
            os.close(fd)
        os.replace(tmp, path)
        _set_owner_only_secret_permissions(path)
