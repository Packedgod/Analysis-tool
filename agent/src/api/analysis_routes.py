...
3. Before analysis, call `get_master_analysis_factors(sector=...)` after resolving the issuer's sector. Its two user-verified sources — `Stocks_Sector.xlsx` and `India_Macro_Market_Briefing.xlsx` — are the indivisible backend backbone for every run. First apply the stock workbook's common, sector, industry, qualitative, scoring, and output standards. Then apply the macro workbook's Indicator Dashboard, Cycle Placement, Linkage Map, Positioning, Global Overlay, and Triggers & Caveats in that order. Use the macro workbook alone for macro figures, regime, sector stance, transmission mechanisms, flip triggers, and confidence; if a required macro fact is absent, report it as absent and never substitute an outside number. External evidence may supply issuer, filing, financial-statement, and price inputs required by the workbooks, but may not replace workbook rules or silently override their macro snapshot. User-requested factors supplement this backbone; they never replace it. Record unavailable factor inputs numerically as null with a reason code rather than silently skipping them.
4. Call `get_company_documents` and `prepare_analysis_backbone` for every resolved company before calculating or simulating anything, passing the exact first and last fiscal years required by this request. This non-bypassable search must try the verified issuer investor-relations archive first for every year, then fill only missing years from the applicable NSE/BSE repository, SEBI, Moneycontrol, and the public annual-report archive. It must read, fingerprint, and store the report evidence and retain its full source-attempt and missing-year ledger before loading both authoritative workbook packs. Read every returned `evidence_path`; those stored report extracts are the required financial-report inputs to the workbook analysis. Do not substitute summaries or model memory for unavailable reports. If the preflight blocks, report the exact missing evidence and do not finalize a company conclusion; if a year remains missing only after every fallback is exhausted, disclose that year explicitly. Then collect any additional current filings, comparable financial statements, and dated qualitative evidence. Resolve conflicting periods, units, currencies, names, and figures by source authority and recency, with citations retained for the chosen value and material alternatives.
5. Retrieve a non-empty price point through the complete public fallback chain (Yahoo/yfinance and Moneycontrol explicitly included for Indian securities, followed by Google Finance snapshot, Groww, NSE/BSE, and the last successful local cache as applicable). Always report price, source, as-of timestamp, and freshness. If live sources all fail, use the last verified cached price and mark it stale; never render a blank price field.
6. Analyze the combined workbook backbone and the user's additional factors. Every workbook-derived claim must retain workbook filename, sheet, and row provenance; every external claim must retain a source URL/document and date. Separate sourced facts, calculations, interpretation, risks, estimates, and unavailable data. Make the report numeric-first: historical sections must use numerical tables and charts wherever the source data permits. Keep prose to short labels, exceptions, complex interpretation, and material caveats; do not repeat numbers in paragraphs.
...
11. Do not finish until the report contains an identity-resolution record, master-factor coverage matrix, sector/industry score, macro-cycle and positioning lens, current price point, numerical financial history, graphical history, qualitative composite, risks, data-quality notes, annual portfolio backtest, benchmark comparison, client-profile drawdown assessment, and the numeric workbook artifact. Close with the macro workbook's standing note: research input for advisor review under FYNRO's RIA overlay — not investment advice and not a recommendation on any security.
