...
5. Analyze the user's factors and separate sourced facts, calculations, interpretation, risks, and estimates. Every material claim must retain a source URL/document and date. Make the report numeric-first: historical sections must use numerical tables and charts wherever the source data permits. Keep prose to short labels, exceptions, complex interpretation, and material caveats; do not repeat numbers in paragraphs.
6. Historical simulation is mandatory and is a long-only portfolio review, not day trading. {strategy} Model stocks as portfolio holdings, use daily bars only for valuation and risk measurement, and hold between scheduled reviews. Re-run the same evidence-and-ranking procedure once per year, then rebalance only at the annual review date unless the verified user strategy requires a less frequent cadence. Include realistic portfolio costs, look-ahead and survivorship controls, benchmark, annual return series, drawdown, volatility, Sharpe, turnover, holdings, and transactions.
7. Tie portfolio drawdown and rebalancing to the client profile. Extract risk tolerance, maximum tolerable drawdown, time horizon, liquidity needs, concentration limits, and benchmark from verified user material or the request. If any is absent, produce clearly labelled conservative profile bands and a conditional comparison rather than claiming a personalized fit. Rebalancing decisions must respect the tightest risk, horizon, liquidity, and concentration constraint.
8. Produce a separate numeric workbook report after the simulation with Summary, Annual, Equity, Holdings, Transactions, Benchmark, Drawdown, STCG, LTCG, Manual Tax, Assumptions, and Charts sheets. Preserve every review-date portfolio state needed for year-over-year and benchmark comparison. The workbook is a required final artifact, not an internal scratch file.
9. Tax is a post-simulation overlay only. First finalize and preserve all pre-tax performance figures. Then separate realized exits into STCG and LTCG by holding period and show each stock separately. Never hard-code a tax rate: leave manual STCG and LTCG rate inputs at zero until the user supplies them, and calculate tax and post-tax gain only from those editable inputs. Do not alter pre-tax backtest metrics.
10. Do not finish until the report contains an identity-resolution record, current price point, numerical financial history, graphical history, qualitative evidence, risks, data-quality notes, annual portfolio backtest, benchmark comparison, client-profile drawdown assessment, and the numeric workbook artifact.
