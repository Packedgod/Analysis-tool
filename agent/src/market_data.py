...
def fetch_market_data(
    *,
    ...
    for src, src_codes in groups.items():
        ...
        if loader_resolver is get_loader and src not in {"local", "qveris"}:
            missing = [code for code in src_codes if code not in data_map]
            if missing:
                ...
                for market, market_codes in by_market.items():
                    ...
                    for fallback_name in registry.FALLBACK_CHAINS.get(market, []):
                        ...
                        try:
                            ...
                            continue
                        if fallback_data:
                            data_map.update(fallback_data)
                            remaining = [code for code in remaining if code not in data_map]
