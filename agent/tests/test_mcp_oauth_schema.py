...
def test_oauth_config_round_trip_snake_and_camel() -> None:
    ...
    camel = MCPOAuthConfig.model_validate(
        {
            ...
            "scopes": ["trading.read"],
            "clientName": "Vantage",
            "cacheDir": "~/.vantage/live/robinhood/oauth",
            "callbackPort": 8765,
            "clientId": "client-id",
    ...
    assert snake == camel
    assert snake.client_name == "Vantage"
...
def test_ibkr_seed_is_official_readonly_oauth_probe() -> None:
    ...
    assert ibkr.auth.cache_dir == "~/.vantage/live/ibkr/oauth"
...
def test_build_client_yields_oauth_streamable_transport() -> None:
    ...
    assert transport.auth._client_name == "Vantage"
