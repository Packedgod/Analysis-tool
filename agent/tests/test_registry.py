...
def test_concurrent_registration_never_exposes_partial_registry(monkeypatch) -> None:
    """A second tool call waits until the complete loader catalog is imported."""
    import importlib
    ...
    def fake_import(name: str):
        ...
        registry.LOADER_REGISTRY[name] = object
        return object()

    monkeypatch.setattr(registry, "_registered", False)
    monkeypatch.setattr(importlib, "import_module", fake_import)
    with patch.dict(registry.LOADER_REGISTRY, {}, clear=True):
        first = threading.Thread(target=registry._ensure_registered)
        second = threading.Thread(target=registry._ensure_registered)
        first.start()
