...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("^NSEI", "yahoo"),  # NIFTY 50 index
        ("RELIANCE", "yahoo"),  # friendly NSE alias -> RELIANCE.NS
        ("HDFCBANK", "yahoo"),
        ("BTC-USDT", "okx"),
        ("ETH/USDT", "ccxt"),
        ("local:my_file", "local"),
...
def test_fetch_normalizes_bare_nse_alias_before_loader_call() -> None:
    out = fetch_market_data(
        codes=["RELIANCE"],
        start_date="2026-01-01",
        end_date="2026-01-02",
...
def test_loader_resolution_error_falls_through_to_unresolved() -> None:
    def broken_resolver(_source):
    ...
    out = fetch_market_data(
        ...
        end_date="2026-01-02",
        source="yahoo",
        loader_resolver=broken_resolver,
    )
    assert out["_unresolved"] == ["^NSEI"]


def test_empty_primary_uses_same_market_runtime_fallback(monkeypatch) -> None:
    ...
    class _WorkingYfinance:
        ...
        def fetch(self, codes, *args, **kwargs):
            ...
            idx.name = "trade_date"
            return {codes[0]: pd.DataFrame({"close": [25_000.0]}, index=idx)}

    monkeypatch.setattr(registry, "_registered", True)
    monkeypatch.setattr(
        registry,
        "get_loader_cls_with_fallback",
        lambda source: _EmptyYahoo,
    )
    monkeypatch.setitem(registry.FALLBACK_CHAINS, "india_equity", ["yahoo", "yfinance"])
    monkeypatch.setitem(registry.LOADER_REGISTRY, "yahoo", _EmptyYahoo)
    monkeypatch.setitem(registry.LOADER_REGISTRY, "yfinance", _WorkingYfinance)
    ...
    assert out["^NSEI"][0]["close"] == 25_000.0
    assert "_unresolved" not in out
