...
def _assert_owner_only_secret_file(path: Path) -> None:
    if os.name != "nt":
    ...
    expected = {win32security.ConvertSidToStringSid(user_sid)}
    account_name = "\\".join(
    ...
    if account_name:
        try:
            ...
            expected.add(win32security.ConvertSidToStringSid(interactive_sid))
        except Exception:
            pass

    actual: set[str] = set()
    for index in range(dacl.GetAceCount()):
        _header, access_mask, ace_sid = dacl.GetAce(index)
        actual.add(win32security.ConvertSidToStringSid(ace_sid))
        assert access_mask & ntsecuritycon.FILE_GENERIC_READ
        assert access_mask & ntsecuritycon.FILE_GENERIC_WRITE
    assert actual == expected


def test_update_settings_writes_env_file_with_owner_only_permissions(
    client: TestClient, tmp_path: Path,
) -> None:
    ...
    assert response.status_code == 200
    _assert_owner_only_secret_file(tmp_path / ".env")
...
def test_atomic_write_secret_creates_0600_file(tmp_path: Path) -> None:
    ...
    _assert_owner_only_secret_file(target)
