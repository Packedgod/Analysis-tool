...
def _client(tmp_path: Path, monkeypatch) -> TestClient:
    # These tests exercise the live-brokerage HTTP surface, which is gated OFF by
    # default (VIBE_TRADING_ENABLE_BROKERAGE). Enable it for the subsystem's own
    # tests, then mount the routes if the default-off import left them absent.
    from src.config.accessor import reset_env_config

    monkeypatch.setenv("VIBE_TRADING_ENABLE_BROKERAGE", "1")
    reset_env_config()
    if not any(getattr(r, "path", None) == "/live/status" for r in api_server.app.routes):
        api_server.register_live_routes(api_server.app)
