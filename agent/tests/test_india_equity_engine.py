...
class TestCanExecute:
    ...
    def test_intraday_interval_allows_same_day_close(self) -> None:
        engine = _engine(interval="15m")
        engine.positions["RELIANCE.NS"] = Position(
            symbol="RELIANCE.NS", direction=1, size=10, entry_price=100.0,
            entry_time=pd.Timestamp("2024-04-01 09:30"),
        )
        bar = _bar()
        bar.name = pd.Timestamp("2024-04-01 10:15")
        assert engine.can_execute("RELIANCE.NS", 0, bar) is True

    def test_daily_interval_still_blocks_same_day_close(self) -> None:
        ...
        bar = _bar()
        bar.name = pd.Timestamp("2024-04-01")
        assert engine.can_execute("RELIANCE.NS", 0, bar) is False
