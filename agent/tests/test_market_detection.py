...
from backtest.engines._market_hooks import _is_china_futures, normalize_market_symbol
...
class TestDetectMarket:
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            ("^NSEI", "india_equity"),  # NIFTY 50 index
            ("^BSESN", "india_equity"),  # SENSEX index
            ("^GSPC", "us_equity"),
            ("^HSI", "hk_equity"),
            # Common bare NSE aliases entered by users
            ("RELIANCE", "india_equity"),
            ("HDFCBANK", "india_equity"),
            ("ICICIBANK", "india_equity"),
            ("INFY", "india_equity"),
            ("TCS", "india_equity"),
            ("ITC", "india_equity"),
            ("LT", "india_equity"),
            ("TATAMOTORS", "india_equity"),
            ("SBIN", "india_equity"),
            ("BHARTIARTL", "india_equity"),
            # Crypto
            ("BTC-USDT", "crypto"),
            ("ETH-USDT", "crypto"),
            ("BTC/USDT", "crypto"),
...
class TestNormalizeCodes:
    ...
    def test_bare_nse_aliases_gain_ns_suffix(self) -> None:
        raw = [
            "RELIANCE", "HDFCBANK", "ICICIBANK", "INFY", "TCS",
            "ITC", "LT", "TATAMOTORS", "SBIN", "BHARTIARTL",
        ]
        assert _normalize_codes(raw, "yahoo") == [f"{code}.NS" for code in raw]

    def test_unknown_bare_symbol_is_not_guessed(self) -> None:
        assert normalize_market_symbol("AAPL") == "AAPL"
