...
must only ever be reachable through an explicit VANTAGE_OCR_ENGINE
...
def test_auto_never_selects_cloud_engine(monkeypatch):
    ...
    monkeypatch.setenv("VANTAGE_OCR_ENGINE", "auto")
    reset_env_config()
...
def test_explicit_qwen_vl_reaches_cloud_factory(monkeypatch):
    ...
    monkeypatch.setenv("VANTAGE_OCR_ENGINE", "qwen-vl")
    reset_env_config()
...
def test_none_disables_ocr(monkeypatch):
    monkeypatch.setenv("VANTAGE_OCR_ENGINE", "none")
    reset_env_config()
...
def test_unknown_choice_falls_back_to_local_only(monkeypatch):
    ...
    monkeypatch.setenv("VANTAGE_OCR_ENGINE", "bogus-engine")
    monkeypatch.setenv("DASHSCOPE_API_KEY", "sk-test-not-real")
    reset_env_config()
