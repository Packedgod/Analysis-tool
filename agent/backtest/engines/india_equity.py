...
Models the Indian cash-equity delivery segment on daily bars and long-only
intraday exits on minute/hour bars. The interval automatically selects the
correct settlement behavior; ``allow_short=True`` remains required to model
intraday (MIS) short entries.
...
class IndiaEquityEngine(BaseEngine):
    ...
    def __init__(self, config: dict):
        ...
        self.interval: str = str(config.get("interval", "1D"))
        self.is_intraday: bool = _is_intraday_interval(self.interval)
    ...
    def can_execute(self, symbol: str, direction: int, bar: pd.Series) -> bool:
        ...
        # 2. T+1 applies to delivery bars, not an intraday (MIS) close.
        if direction == 0 and not self.is_intraday:
...
def _is_intraday_interval(interval: str) -> bool:
    """Return whether a configured bar interval is finer than one day."""
