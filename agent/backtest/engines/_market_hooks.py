...
# Common NSE symbols that users naturally enter without Yahoo's ``.NS``
# suffix. Bare alphabetic tickers are otherwise ambiguous (``AAPL`` could be
# US, while ``TCS`` is NSE), so only explicit aliases are normalized. Keep
# this as an alias table rather than labelling it a current index-constituent
# list; index membership changes while these exchange symbols remain stable.
_NSE_BARE_ALIASES = frozenset(
    {
        "RELIANCE",
        "HDFCBANK",
        "ICICIBANK",
...
def normalize_market_symbol(code: str) -> str:
    """Normalize unambiguous user-facing aliases to provider symbols.

    Common bare NSE tickers are converted to Yahoo's ``.NS`` convention.
    Already-qualified symbols and unknown/ambiguous values are preserved
    apart from surrounding whitespace.
    """
    normalized = str(code).strip()
    upper = normalized.upper()
    if upper in _NSE_BARE_ALIASES:
        return f"{upper}.NS"
    return normalized
...
_MARKET_PATTERNS = [
    # Yahoo-style index symbols.  These must precede the catch-all default;
    # otherwise ``^NSEI`` is misclassified as an A-share and routed to
    # Tushare/AkShare instead of India's Yahoo providers.
    (re.compile(r"^\^(NSEI|NSEBANK|CNXIT|BSESN)$", re.I), "india_equity"),
    (re.compile(r"^\^(GSPC|DJI|IXIC|RUT|VIX)$", re.I), "us_equity"),
    (re.compile(r"^\^(HSI|HSCE)$", re.I), "hk_equity"),
...
def _detect_market(code: str) -> str:
    ...
    """
    normalized = normalize_market_symbol(code)
