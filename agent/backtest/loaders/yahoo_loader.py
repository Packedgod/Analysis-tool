"""Yahoo Finance loader: free, no-auth global equity/index OHLCV via direct HTTP.
...
IP-rate-limiting us. Covers US/HK/India equities and Yahoo index symbols such
as ``^NSEI``; the client maps each project symbol to Yahoo's ticker form.
...
def _is_supported(code: str) -> bool:
    ...
    return normalized.startswith("^") or normalized.endswith((".US", ".HK", ".NS", ".BO"))
