...
def _ensure_registered() -> None:
    ...
    if _registered:
        return
    # Registration is reached from parallel tool calls.  Marking the registry
    # ready before the imports completed let a second thread observe a partial
    # registry and reject valid names (most visibly ``source='yahoo'``).
    with _registration_lock:
        ...
        _loader_modules = [
            ...
            "backtest.loaders.nse_loader",
            "backtest.loaders.moneycontrol_loader",
            "backtest.loaders.groww_loader",
...
FALLBACK_CHAINS: dict[str, list[str]] = {
    ...
    # Verified public India sources, ordered by reachability then authority:
    # Yahoo's throttle-tolerant chart endpoint leads (deep history, no key), then
    # Groww's public login-free feed. The official NSE feed and Moneycontrol are
    # the most authoritative but sit deeper because their WAFs frequently block
    # non-residential IPs (503/403) — when reachable they still backstop gaps and
    # confirm prints. yfinance and the local bridge trail. The broker-history
    # bridge is intentionally gone: India data comes from public feeds, never a
    # broker login.
    "india_equity": ["yahoo", "groww", "nse", "moneycontrol", "yfinance", "local"],
