...
type RequestOptions = RequestInit & { networkRetries?: number };
...
async function request<T>(path: string, options?: RequestOptions): Promise<T> {
  const { headers, networkRetries = 0, ...rest } = options ?? {};
  ...
  for (let attempt = 0; attempt <= networkRetries; attempt += 1) {
    try {
      ...
      if (attempt >= networkRetries) {
        throw new ApiError(
          "The local service is restarting and did not accept the message. Please retry in a moment.",
          0,
        );
      }
  ...
  if (!res) {
    throw new ApiError("The local service did not return a response.", 0);
  }
  if (!res.ok) {
...
export const api = {
  ...
  sendMessage: (sid: string, content: string) => {
    ...
    return request<{ message_id: string; attempt_id: string }>(`/sessions/${sid}/messages`, {
      method: "POST",
...
export interface ApiInfo {
  ...
  version: string;
  capabilities?: {
    /** Whether the live-brokerage subsystem is enabled (default false). */
    brokerage?: boolean;
  };
}
