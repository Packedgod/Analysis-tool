...
/**
 * Max characters in one chat message. Mirrors the server's
 * `sessions_routes.MAX_MESSAGE_CHARS` so an over-long paste gets an actionable
 * message here instead of a bare 422 from the API. Keep the two in sync.
 *
 * This is the measured provider ceiling (~150k tokens); beyond it the model
 * itself returns `context_too_large`. Bulk data should be uploaded as a context
 * file, which the agent reads back in bounded windows.
 */
const MAX_MESSAGE_CHARS = 600_000;

/* ---------- Base-context files ----------
 * An uploaded file is session-scoped working data, not a one-shot attachment:
 * it stays listed until removed, survives a page reload, and is re-announced to
 * the agent on every message so it can read it at any point in the conversation.
 */
...
/** localStorage key holding one session's base-context files. */
const contextFilesKey = (sessionId: string | null) =>
  `vt.contextFiles.${sessionId ?? "new"}`;

function loadContextFiles(sessionId: string | null): ContextFile[] {
  try {
    const raw = localStorage.getItem(contextFilesKey(sessionId));
    const parsed = raw ? JSON.parse(raw) : null;
    if (!Array.isArray(parsed)) return [];
    // Defensive: tolerate hand-edited / stale entries without crashing the page.
    return parsed.filter(
      (f) => f && typeof f.filename === "string" && typeof f.filePath === "string",
    );
  } catch {
    return [];
  }
...
function saveContextFiles(sessionId: string | null, files: ContextFile[]): void {
  try {
    if (files.length) {
      localStorage.setItem(contextFilesKey(sessionId), JSON.stringify(files));
    } else {
      localStorage.removeItem(contextFilesKey(sessionId));
    }
  } catch {
    // A full/blocked quota must never break sending a message.
  }
}

/**
 * Prefix announcing the session's base-context files to the agent.
 *
 * Sent with every message so the files remain reachable for the whole
 * conversation; the agent reads them on demand rather than having their
 * contents inlined into the prompt.
 */
function contextFilesPrefix(files: ContextFile[]): string {
  if (!files.length) return "";
  const lines = files
    .map((f, i) => `${i + 1}. ${f.filename} — ${f.filePath}`)
    .join("\n");
  return (
    `[Session context files — attached by the user as base data for this whole conversation. ` +
    `Read any of them with the read_document tool (Excel .xlsx/.xls incl. all sheets, CSV/TSV, ` +
    `PDF, Word, PowerPoint, JSON, text). Use them as the primary source when the question ` +
    `relates to their contents:]\n${lines}\n\n`
  );
}
...
export function Agent() {
  const { t } = useTranslation();
  // Live-brokerage master switch (default off): gates every connector/mandate/
  // runner surface below so the research-only build shows no live-trading UI.
  ...
  /* Base-context files: uploaded documents/datasets that stay attached to the
   * session and are announced to the agent on EVERY message (not consumed by
   * the first send). The agent reads them on demand with read_document, so an
   * Excel/CSV/PDF stays available as the conversation's working data. */
  ...
  /* Base-context files are per-session and survive a reload: rehydrate when the
   * active session changes, and persist on every mutation. */
  const contextLoadedForRef = useRef<string | null | undefined>(undefined);

  useEffect(() => {
    let files = loadContextFiles(sessionId);
    // Files attached before the first message are staged under the "new" key;
    // once the session exists, carry them over so they aren't lost when the
    // session id appears.
    if (!files.length && sessionId) {
      const staged = loadContextFiles(null);
      if (staged.length) {
        files = staged;
        saveContextFiles(sessionId, staged);
        saveContextFiles(null, []);
      }
    }
    setAttachments(files);
    contextLoadedForRef.current = sessionId;
  }, [sessionId]);

  useEffect(() => {
    // Persist only after this session's files have been rehydrated, so the
    // initial empty state can never wipe what's in storage.
    if (contextLoadedForRef.current !== sessionId) return;
    saveContextFiles(sessionId, attachments);
  ...
  const refreshLiveStatus = useCallback(async () => {
    // Research-only build: the /live routes are not mounted, so skip the poll
    // entirely rather than hammer a 404 every cycle.
    if (!brokerageEnabled) {
      setLiveStatus(null);
  ...
  const runPrompt = async (prompt: string) => {
    ...
    // Base-context files are announced to the agent on every message and
    // deliberately NOT cleared — they stay attached until the user removes
    // them. The announcement is kept out of the rendered bubble (the file chips
    // above the composer already show what's attached), so it doesn't repeat as
    // noise in every message the user sees.
    const sentPrompt = `${contextFilesPrefix(attachments)}${finalPrompt}`;
    // Guard the server's cap here so an over-long paste gets an actionable
    // message and keeps its text, instead of a bare 422. Bulk data belongs in
    // an uploaded context file, which is the suggested remedy.
    if (sentPrompt.length > MAX_MESSAGE_CHARS) {
      toast.error(t("agent.messageTooLong", { max: MAX_MESSAGE_CHARS.toLocaleString() }));
      return;
    }
    setInput("");
    ...
    try {
      ...
      setupSSE(sid);
      const sent = await api.sendMessage(sid, sentPrompt);
      void syncCompletedAttempt(sid, sent.attempt_id);
    } catch (error) {
      act().setStatus("error");
      ...
      // Preserve the user's text when the local service could not accept it.
      setInput(prompt);
      toast.error(message);
  ...
  const handleFileSelect = useCallback(async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!files.length) return;
    e.target.value = "";
    // Mirrors the server-side blocklist (uploads_routes._BLOCKED_UPLOAD_EXT) so
    // the user gets an instant, specific rejection instead of a round-trip 400.
    // The server remains the authority; this is convenience, not the control.
    const blockedExts = [
    ...
    try {
      // Upload sequentially: one bad file reports itself and never aborts the rest.
      for (const file of files) {
        ...
        try {
          ...
          setAttachments((prev) => {
            // Re-uploading the same name replaces it rather than duplicating.
            const next = prev.filter((f) => f.filename !== result.filename);
            return [...next, { filename: result.filename, filePath: result.file_path }];
          });
      ...
      <div ref={listRef} className="flex-1 overflow-auto p-6 scroll-smooth relative">
        <div className="max-w-3xl mx-auto space-y-4">
          ...
          {timelineRows.map((row, rowIdx) => {
            if (row.render === "live") {
              // Research-only build: never surface live-trading artifacts even
              // if a stale SSE frame slipped through.
              if (!brokerageEnabled) return null;
      ...
      <form onSubmit={handleSubmit} className="border-t p-4 bg-background/80 backdrop-blur-sm">
        <div className="max-w-3xl mx-auto space-y-2">
          ...
          {/* Persistent live runtime status panel — sits alongside the goal/mandate
              ...
              and is omitted entirely in the research-only (brokerage-off) build. */}
          {brokerageEnabled && (
            <RunnerStatus
              status={liveStatus}
          ...
          {/* Base-context file chips — stay attached for the whole session. */}
          {attachments.length > 0 && (
            <div className="flex flex-wrap items-center gap-1">
              <span className="text-[11px] text-muted-foreground mr-0.5">
                {t("agent.contextFiles")}
              </span>
              {attachments.map((file) => (
                ...
                >
                  ...
                  <button
                    type="button"
                    aria-label={t("agent.removeContextFile", { filename: file.filename })}
                    onClick={() =>
                      setAttachments((prev) => prev.filter((f) => f.filePath !== file.filePath))
                    }
          ...
          <div className="flex gap-2 items-end">
            ...
            <input
              ...
              type="file"
              multiple
              /* Data/document formats accepted as base context. Spreadsheets and
               * columnar/data formats lead since they're the common "here is my
               * dataset" case. Executables/scripts/archives are rejected by the
               * server and deliberately absent here. */
              accept=".xlsx,.xls,.xlsm,.xlsb,.ods,.csv,.tsv,.parquet,.feather,.arrow,.db,.sqlite,.sqlite3,.json,.jsonl,.ndjson,.pdf,.docx,.doc,.pptx,.txt,.md,.log,.toml,.ini,.cfg,.html,.htm,.xml,.rst,.sql,.dat,.png,.jpg,.jpeg,.gif,.bmp,.webp,.tiff,.tif"
            ...
            {status === "streaming" ? (
              ...
              <button
                type="submit"
                disabled={goalComposerActive ? !input.trim() : (!input.trim() && attachments.length === 0)}
