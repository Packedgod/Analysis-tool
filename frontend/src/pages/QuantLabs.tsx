...
import { AsOfControl } from "@/components/common/AsOfControl";
import { cn } from "@/lib/utils";

/**
 * Labs whose results derive from fetched market history, and can therefore be
 * bounded by a point-in-time date. The pure simulations (option pricing, order
 * book) take their inputs from the form, so a clock would mean nothing there.
 */
const AS_OF_LABS = new Set([
  "backtest", "pairs", "portfolio", "monte-carlo", "factor-model", "market-dashboard",
]);
...
function normalizePayload(values: Record<string,string|number>): Record<string, unknown> {
...
export function QuantLabs(){
  ...
  // Point-in-time study date. Only the labs that fetch market history can honour
  // it; the pure simulations (option pricing, order book) have no history to bound.
  const [asOf,setAsOf]=useState<string|null>(null);
  const supportsAsOf=AS_OF_LABS.has(activeId);
  ...
  async function run(){setLoading(true);try{const payload=normalizePayload(values);if(supportsAsOf&&asOf)payload.as_of=asOf;setResult(await api.runQuantLab(active.id,payload));}catch(e){toast.error(e instanceof Error?e.message:"Lab failed")}finally{setLoading(false)}}
  return <div className="h-full overflow-y-auto bg-background"><div className="mx-auto max-w-[1540px] px-5 py-7 lg:px-8">
    ...
    <section className="mt-6 grid gap-6 xl:grid-cols-[340px_minmax(0,1fr)]"><aside className="terminal-card h-fit rounded-[22px] border p-5 xl:sticky xl:top-5"><div className="font-mono text-[10px] uppercase tracking-[.2em] text-primary">Configure engine</div><h2 className="mt-2 text-xl font-semibold">{active.name}</h2><p className="mt-2 text-sm leading-6 text-muted-foreground">{active.description}</p><div className="mt-5 space-y-3">{active.fields.map(f=><label key={f.key} className="block"><span className="mb-1.5 block text-xs text-muted-foreground">{f.label}</span>{f.type==="select"?<select value={values[f.key]} onChange={e=>setValues(v=>({...v,[f.key]:e.target.value}))} className="h-10 w-full rounded-xl border bg-background px-3 text-sm">{f.options?.map(o=><option key={o}>{o}</option>)}</select>:<input type={f.type==="number"?"number":"text"} step="any" value={values[f.key]??""} onChange={e=>setValues(v=>({...v,[f.key]:f.type==="number"?Number(e.target.value):e.target.value}))} className="h-10 w-full rounded-xl border bg-background px-3 text-sm outline-none focus:border-primary/50"/>}</label>)}</div>{supportsAsOf&&<AsOfControl className="mt-4" value={asOf} onChange={setAsOf} max={new Date().toISOString().slice(0,10)}/>}<button onClick={()=>void run()} disabled={loading} className="mt-5 flex h-11 w-full items-center justify-center gap-2 rounded-xl bg-primary font-semibold text-primary-foreground transition hover:brightness-110 disabled:opacity-60">{loading?<Loader2 className="h-4 w-4 animate-spin"/>:<Play className="h-4 w-4 fill-current"/>}{loading?"Running evidence pipeline…":"Run analysis"}</button></aside>
